...
//        live: 'disabled',
        ...
        fields: {
            ...
            isi: {
                validators: {
                    notEmpty: {
                        message: 'Isi E-Akademik harus diisi'
                    },
                }
            },
            guru: {
              ...
              <div id="isi">
				...
				<input type="hidden" name="kode_lama" value="<?php echo isset($b)?$b['id_akademik']:''; ?>" />
							<div class="form-group">
                            	<label class="col-lg-3 control-label">Akademik</label>
                                <div class="col-lg-5">
									<input type="text" name="id_akademikg" class="form-control" value="<?php echo isset($b)?$b['id_akademik']:''; ?>" maxlength="50" />
							  	</div>
							</div>
                            <div class="form-group">
                                ...
                                <div class="col-lg-5">
                                    <select name="guru" class="form-control">
                                        <?php
                                        $m = mysql_query("select * from guru order by nama_guru asc");
                                        while($n = mysql_fetch_array($m)){
                                        ?>
                                            <option value="<?php echo $n['id_guru']; ?>" <?php echo isset($b)&&$b['id_guru']==$n['id_guru']?'selected':''; ?>><?php echo $n['nama_guru']; ?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                ...
                <table id="data" class="table table-bordered table-hover" cellspacing="0" width="100%">
                    ...
                    <tbody>
                    	<?php 
                        $x = mysqli_query($con, "select * from akademik a join guru g on a.id_guru=g.id_guru order by tgl_posting desc");
